# Simulate Weibull random fields with Clayton copula
...
while(k <= n_sim) {
  ...
  fixed <- list(sill = sill, smooth = smooth, power2 = power2, nugget = nugget, nu = nu)
...
# Save results as CSV
write.csv(res, file = file.path(output_dir, "weibull_clayton_copula_simulations.csv"), 
          row.names = FALSE)
write.csv(results_summary, file = file.path(output_dir, "weibull_clayton_copula_summary.csv"), 
          row.names = FALSE)
...
cat("- data/processed/weibull_clayton_copula_simulations.csv\n")
cat("- data/processed/weibull_clayton_copula_summary.csv\n")
