...
timestamp_v2 <- function() format(Sys.time(), "%Y%m%d_%H%M%S")
run_tag <- ifelse(is.null(args$tag), timestamp_v2(), as.character(args$tag))
...
add_job_v2 <- function(job_id, kind, value, script, script_args) {
...
common_args_v2 <- function(reps, chunk_size) {
  out <- c(
...
if (mode %in% c("all", "simulation")) {
  for (i in seq_along(sim_groups)) {
    group <- sim_groups[[i]]
    jobs[[length(jobs) + 1L]] <- add_job_v2(
      job_id = paste0("simulation_group_", i),
      kind = "simulation",
      value = group,
      script = "src/run_simulation_study_v2.R",
      script_args = c(common_args_v2(sim_reps_v2, sim_chunk_size_v2), "--scenario", group)
    )
  }
}
...
if (mode %in% c("all", "coverage")) {
  for (m in m_values) {
    jobs[[length(jobs) + 1L]] <- add_job_v2(
      ...
      script = "src/run_coverage_study_v2.R",
      script_args = c(
        common_args_v2(coverage_reps_v2, coverage_chunk_size_v2),
        "--scenario", "S0",
...
message("External parallel v2")
message("Mode: ", mode)
message("Simulation replicates: ", sim_reps_v2)
message("Coverage replicates: ", coverage_reps_v2)
...
status <- system2(rscript_v2(), "src/summarize_simulation_study_v2.R")
if (!identical(status, 0L)) stop("Summary step failed.")

message("External parallel v2 complete.")
